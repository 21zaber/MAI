...
class MainWindow(QtGui.QMainWindow):
    ...
    def set_pixmaps(self):
        ...
        path_res = './lab_5_1/gui/resource/'

        pixmap_labels = [[w.label_1_1, w.label_1_2, w.label_1_3, w.label_1_4],
                         [w.label_2_1, w.label_2_2],
                         [w.label_3_1, w.label_3_2],
                         [w.label_4_1],
                         [w.label_5_1],
                         [w.label_6_1, w.label_6_2, w.label_6_3, w.label_6_4],
    ...
    def save_args(self):
        w = self.centralWidget()
        self.main_args = [w.doubleSpinBox_a.value(),
        ...
        self.initial_func = FunctionFromStr(w.lineEdit_psi.text())
        self.initial_func.set_args_name('x')
        self.initial_func(0)
    ...
    def update_tab_page(self, index):
        ...
        if self.args_valid():
            ...
            self.update_canvas_solution()
            sigma = self.main_args[0] * self.step_t / self.step_x ** 2
            if sigma > 0.5:
                msg = 'sigma = {0} * {1} / {2}^2 = \n\t = {3} > 0.5\nЯвная схема неустойчива'.format(self.main_args[0],
                                                                                                     self.step_t,
                                                                                                     self.step_x,
                                                                                                     sigma)
    ...
    def args_valid(self):
        a, b, c, f = self.main_args
        alpha, beta, phi_0 = self.left_boundary_args
        gamma, delta, phi_1 = self.right_boundary_args
        return (a > 0 and
                f.valid and phi_0.valid and phi_1.valid and self.initial_func.valid and
    ...
    def update_solutions(self):
        solver = ParabolicPDE(*(self.main_args +
                                self.left_boundary_args +
                                self.right_boundary_args +
                                [self.initial_func, self.min_x, self.max_x, self.max_t]))

        self.solutions = [[None] * len(self.app_order_names) for _ in range(len(self.scheme_names))]
        solve_progress_dialog = QtGui.QProgressDialog('Solving...', 'cancel', 0,
                                                      len(self.scheme_names) * len(self.app_order_names))
    ...
    def update_canvas_solution(self):
        ...
        for i, s in enumerate(scheme):
            ...
            for j, a in enumerate(app_order):
                if a[0].isChecked():
                    self.canvas_solution.add(x, self.solutions[i][j][k], label=s[1] + ' ' + a[1])
